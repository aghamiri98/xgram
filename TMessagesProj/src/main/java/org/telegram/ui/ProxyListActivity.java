...
import sections.intefaces.OnAntiFilterClickListener;
...
public class ProxyListActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
   ...
   /*private static OnAntiFilterClickListener onAntiFilterClickListener;

    public static void setOnAntiFilterClickListener(OnAntiFilterClickListener onAntiFilterClickListener) {
        onAntiFilterClickListener = onAntiFilterClickListener;
    }*/
    ...
    private final int SHARE_PROXY_MENU = 13;
    private final int DELETE_UNAVILABLE_PROXY_MENU = 17;
    //MoreMenu->
    int menu_item_share_proxy = 205;
    int menu_item_share_avialable_proxy = 206;
    int menu_item_delete_unavialable_proxy = 207;


    //
    //AutomaticProxy->
    private int smartProxyRow;
    private boolean smartProxySettings;
    private int autoProxyRow;
    private boolean autoProxyRowSettings;
    private int sortPingRow;
    private boolean sortPingRowSettings;
    ...
    public boolean onFragmentCreate() {
        ...
        //AutomaticProxy->
        smartProxySettings = preferences.getBoolean("smartProxySettings", false) && !SharedConfig.proxyList.isEmpty();

        autoProxyRowSettings = preferences.getBoolean("proxy_anty_filter_enabled", true);


        sortPingRowSettings = preferences.getBoolean("sort_ping_proxy", true);
        if (sortPingRowSettings) {
            sortByPingTime();
        }
    ...
    public View createView(Context context) {
        ...
        actionBar.setActionBarMenuOnItemClick(new ActionBar.ActionBarMenuOnItemClick() {
            ...
            public void onItemClick(int id) {
                ...
                //AutomaticProxy->
                else if (id == menu_item_share_proxy) {
                    ArrayList<SharedConfig.ProxyInfo> proxyInfos = SharedConfig.proxyList;
                    String str = "";
                    for (SharedConfig.ProxyInfo proxyInfo : proxyInfos) {
                        str = str + "https://t.me/proxy?server=" + proxyInfo.address + "&port=" + proxyInfo.port + "&secret=" + proxyInfo.secret + "\n\n";
                    }
                    Utils.shareAlert(context, "Share Proxies", str);
                } else if (id == menu_item_share_avialable_proxy) {
                    /*if (!PromoteHelper.isPromotedUser()){
                            Utils.toast(LocaleController.getString("ForAvailableUpgradeAccount", R.string.ForAvailableUpgradeAccount));
                            return;
                    }*/
                    ...
                    Utils.shareAlert(context, "Share Avialable Proxies", str);
                } else if (id == menu_item_delete_unavialable_proxy) {
                    /*if (!PromoteHelper.isPromotedUser()){
                        Utils.toast(LocaleController.getString("ForAvailableUpgradeAccount", R.string.ForAvailableUpgradeAccount));
                        return;
                    }*/
                    ArrayList<SharedConfig.ProxyInfo> proxyInfos = SharedConfig.proxyList;
                    ArrayList<SharedConfig.ProxyInfo> proxyInfos2 = new ArrayList<>();
                    for (SharedConfig.ProxyInfo proxyInfo : proxyInfos) {
                        if (!proxyInfo.available) {
                            proxyInfos2.add(proxyInfo);
                        }

                    }
                    //SharedConfig.proxyList.clear();
                    SharedConfig.proxyList.removeAll(proxyInfos2);
                    SharedConfig.saveProxyList();
                    updateRows(true);

                    /*ArrayList<SharedConfig.ProxyInfo> proxyInfos = SharedConfig.proxyList;
                    for(Iterator<SharedConfig.ProxyInfo> it = proxyInfos.iterator(); it.hasNext();) {
                        SharedConfig.ProxyInfo element = it.next();
                        if(!element.available) {
                            //irrelevant stuff..
                            SharedConfig.proxyList.remove(element);
                        }
                    }
        ...
        //menu.addItemWithWidth(SHARE_PROXY_MENU, R.drawable.share, AndroidUtilities.dp(56));
        //menu.addItemWithWidth(DELETE_UNAVILABLE_PROXY_MENU, R.drawable.chats_delete, AndroidUtilities.dp(56));
        ...
        headerItem.addSubItem(menu_item_share_proxy, LocaleController.getString("shareProxies", R.string.shareProxies));
        headerItem.addSubItem(menu_item_share_avialable_proxy, LocaleController.getString("shareAvialableProxies", R.string.shareAvialableProxies));
        headerItem.addSubItem(menu_item_delete_unavialable_proxy, LocaleController.getString("deleteUnavilableProxy", R.string.deleteUnavilableProxy));
        //AutomaticProxy/
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == useProxyRow) {
                ...
                //AutomaticProxy->
                    ...
                    RecyclerListView.Holder holder = (RecyclerListView.Holder) listView.findViewHolderForAdapterPosition(autoProxyRow);
                    if (holder != null) {
                        ((TextCheckCell) holder.itemView).setChecked(false);
                        autoProxyRowSettings=false;
                    }

                    RecyclerListView.Holder holder3 = (RecyclerListView.Holder) listView.findViewHolderForAdapterPosition(sortPingRow);
                    if (holder != null) {
                        ((TextCheckCell) holder3.itemView).setChecked(false);
                        sortPingRowSettings=false;

                    }

                    RecyclerListView.Holder holder2 = (RecyclerListView.Holder) listView.findViewHolderForAdapterPosition(smartProxyRow);
                    if (holder2 != null) {
                        ((TextCheckCell) holder2.itemView).setChecked(false);
                        smartProxySettings=false;
                    }
                    ProxyListActivity.this.autoProxyRowSettings = false;
                }
                //AutomaticProxy/
            ...
            //AutomaticProxy->
            else if (position == smartProxyRow) {
                smartProxySettings = smartProxySettings ^ true;
                ((TextCheckCell) view).setChecked(smartProxySettings);
                SharedPreferences preferences = MessagesController.getGlobalMainSettings();
                SharedPreferences.Editor editor = MessagesController.getGlobalMainSettings().edit();
                editor.putBoolean("smartProxySettings", smartProxySettings);
                editor.commit();
            } else if (position == autoProxyRow) {
                autoProxyRowSettings = ProxyListActivity.this.autoProxyRowSettings ^ true;
                ((TextCheckCell) view).setChecked(ProxyListActivity.this.autoProxyRowSettings);
                SharedPreferences preferences = MessagesController.getGlobalMainSettings();
                SharedPreferences.Editor editor = MessagesController.getGlobalMainSettings().edit();
                editor.putBoolean("proxy_anty_filter_enabled", autoProxyRowSettings);
                editor.commit();

                ProxyHandler.getLastProxy();

                if (autoProxyRowSettings){
                    //onAntiFilterClickListener.onClick();

                   /* if (onAntiFilterClickListener!=null){

                    }*/
                }


                    /*if (MessagesController.getGlobalMainSettings().contains(Const.PROXY_ANTY_FILTER_ENABLED)){
                        ProxyHandler.getLastProxy();
                    }*/
                if (autoProxyRowSettings) {
                    ...
                    if (holder != null) {
                        ...
                        editor3.putBoolean("proxy_enabled", useProxySettings);
                        editor3.commit();
                    }
                }else {
                    /*RecyclerListView.Holder holder = (RecyclerListView.Holder) listView.findViewHolderForAdapterPosition(useProxyRow);
            ...
            } else if (position == sortPingRow) {

                sortPingRowSettings = !sortPingRowSettings;
                ((TextCheckCell) view).setChecked(sortPingRowSettings);
                SharedPreferences preferences = MessagesController.getGlobalMainSettings();
                SharedPreferences.Editor editor = MessagesController.getGlobalMainSettings().edit();
                editor.putBoolean("sort_ping_proxy", sortPingRowSettings);
                editor.commit();
                if (sortPingRowSettings == true) {
                    sortByPingTime();
    ...
    private void updateRows(boolean notify) {
        ...
        //AutomaticProxy->
        smartProxyRow = rowCount++;
        autoProxyRow = rowCount++;
        sortPingRow = rowCount++;
        //AutomaticProxy/
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
            ...
            switch (holder.getItemViewType()) {
                ...
                case 3: {
                    ...
                    //AutomaticProxy->
                        ...
                        checkCell.setTextAndCheck(LocaleController.getString("smartProxySettings", R.string.smartProxySettings), smartProxySettings, false);
                        return;
                    } else if (position == autoProxyRow) {
                        checkCell.setTextAndCheck(LocaleController.getString("UseAutoProxy", R.string.UseAutoProxy), autoProxyRowSettings, false);
                        return;
                    } else if (position == sortPingRow) {
                        checkCell.setTextAndCheck(LocaleController.getString("SortPing", R.string.SortPing), sortPingRowSettings, false);
                        return;
                    }
        ...
        public void onViewAttachedToWindow(RecyclerView.ViewHolder holder) {
            ...
            if (viewType == 3) {
                ...
                //AutomaticProxy->
                    ...
                    checkCell.setChecked(smartProxySettings);
                } else if (position == autoProxyRow) {
                    checkCell.setChecked(autoProxyRowSettings);
                } else if (position == sortPingRow) {
                    checkCell.setChecked(sortPingRowSettings);
                }
        ...
        public boolean isEnabled(RecyclerView.ViewHolder holder) {
            int position = holder.getAdapterPosition();
//AutomaticProxy->
            return position == useProxyRow || position == smartProxyRow || position == autoProxyRow || position == sortPingRow || position == callsRow || position == proxyAddRow || position >= proxyStartRow && position < proxyEndRow;
            //AutomaticProxy/
        }
        ...
        public int getItemViewType(int position) {
            ...
            //AutomaticProxy->
            else if (position == useProxyRow || position == smartProxyRow || position == autoProxyRow || position == sortPingRow || position == callsRow) {
                return 3;
    ...
    //AutomaticProxy->
    private void sortByPingTime() {
//ProxyPingTimeComparator proxyPingTimeComparator=new ProxyPingTimeComparator();
//proxyPingTimeComparator.compare()
        Collections.sort(SharedConfig.proxyList);
        updateRows(true);
    }

    public class ProxyPingTimeComparator implements Comparator<SharedConfig.ProxyInfo> {


        @Override
        public int compare(SharedConfig.ProxyInfo o1, SharedConfig.ProxyInfo o2) {
            if (o1.ping >= o2.ping) {
                return 0;
            }
