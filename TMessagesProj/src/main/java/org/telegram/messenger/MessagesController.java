...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsSpecialHides = new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsSpecialBots = new ArrayList<>();
    ...
    public ArrayList<TLRPC.TL_dialog> dialogsadmins = new ArrayList<>();
    public ArrayList<TLRPC.TL_dialog> dialogsUnread = new ArrayList<>();
    public ArrayList<Long> hid = new ArrayList<>();
    ...
    public MessagesController(int num) {
        ...
        //Devgram
        SharedPreferences plusPrefs = ApplicationLoader.applicationContext.getSharedPreferences(Theme.CONFIG_PREF_NAME, Activity.MODE_PRIVATE);
...
//Devgram
    ...
    public void processUpdates(final TLRPC.Updates updates, boolean fromQueue) {
        ...
        if (updates instanceof TLRPC.TL_updateShort) {
            ...
            if (user != null && user.status != null && user.status.expires <= 0) {
                ...
                if(BuildConfig.DEBUG && id != UserConfig.getInstance(currentAccount).getClientUserId() && (Theme.plusShowOnlineToast || Theme.plusShowOfflineToast) && !AndroidUtilities.playingAGame) {
                    AndroidUtilities.runOnUIThread(new Runnable() {
                        @Override
                        public void run() {
                            NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.showStatusNotifications, id, false);
                        }
                    });
                }
                //
                ...
                for (int a = 0, size = updatesOnMainThreadFinal.size(); a < size; a++) {
                    ...
                    if (baseUpdate instanceof TLRPC.TL_updatePrivacy) {
                        ...
                        if (currentUser != null) {
                            ...
                            //Devgram
                            if(update.user_id != UserConfig.getInstance(currentAccount).getClientUserId() && (Theme.plusShowOnlineToast || (BuildConfig.DEBUG && Theme.plusShowOfflineToast)) && !AndroidUtilities.playingAGame){
                                NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.showStatusNotifications, currentUser.id, false);
                            }
                            //
    ...
    public void sortDialogs2(SparseArray<TLRPC.Chat> chatsDict) {
        dialogsServerOnly.clear();
        dialogsCanAddUsers.clear();
        dialogsChannelsOnly.clear();
        dialogsGroupsOnly.clear();
    ...
    public void updateTimerProc2() {
        ...
        checkDeletingTask(false);
        if (UserConfig.getInstance(currentAccount).isClientActivated()) {
            if (ConnectionsManager.getInstance(currentAccount).getPauseTime() != 0 || !ApplicationLoader.isScreenOn || ApplicationLoader.mainInterfacePaused) {
                if (!(this.statusSettingState == UPDATE_MASK_AVATAR || offlineSent || Math.abs(System.currentTimeMillis() - ConnectionsManager.getInstance(currentAccount).getPauseTime()) < 2000)) {
                    statusSettingState = UPDATE_MASK_AVATAR;
                    if (this.statusRequest != 0) {
                        ConnectionsManager.getInstance(currentAccount).cancelRequest(this.statusRequest, true);
                    }
                    ...
                    statusRequest = ConnectionsManager.getInstance(currentAccount).sendRequest(tL_account_updateStatus, new RequestDelegate() {
                        public void run(TLObject tLObject, TLRPC.TL_error tL_error) {
                            if (tL_error == null) {
                                MessagesController.this.offlineSent = true;
                            } else if (MessagesController.this.lastStatusUpdateTime != 0) {
                                MessagesController.this.lastStatusUpdateTime = lastStatusUpdateTime + 5000;
                            }
        ...
        if (!(this.channelViewsToSend.size() == 0) && Math.abs(System.currentTimeMillis() - lastViewsCheckTime) >= 5000) {
            lastViewsCheckTime = System.currentTimeMillis();
            intValue = 0;
            while (intValue < UPDATE_MASK_AVATAR) {
                SparseArray sparseArray = channelViewsToSend;
                if (sparseArray.size() != 0) {
                    ...
                    while (i3 < sparseArray.size()) {
                        ...
                        ConnectionsManager.getInstance(currentAccount).sendRequest(tL_messages_getMessagesViews, new RequestDelegate() {
                            @Override
                            public void run(TLObject response, TLRPC.TL_error error) {
                                if (error == null) {
                                    ...
                                    MessagesStorage.getInstance(currentAccount).putChannelViews(channelViews, tL_messages_getMessagesViews.peer instanceof TLRPC.TL_inputPeerChannel);
                                    AndroidUtilities.runOnUIThread(new Runnable() {
                                        @Override
                                        public void run() {
        ...
        if (!this.onlinePrivacy.isEmpty()) {
            arrayList = null;
            intValue = ConnectionsManager.getInstance(currentAccount).getCurrentTime();
            for (ConcurrentHashMap.Entry<Integer, Integer> entry : onlinePrivacy.entrySet()) {
            ...
            if (arrayList != null) {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    public void run() {
                        NotificationCenter instance = NotificationCenter.getInstance(currentAccount);
