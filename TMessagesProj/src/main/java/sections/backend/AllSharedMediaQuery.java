...
public class AllSharedMediaQuery {
    ...
    public static void loadMedia(final long uid, final int offset, final int count, final int max_id, final int type, final boolean fromCache, final int classGuid) {
       // final boolean isChannel = (int) uid < 0 && ChatObject.isChannel(-(int) uid);
        final boolean isChannel = (int) uid < 0 && ChatObject.isChannel(-(int) uid,Const.currentAccount);
        ...
        if (fromCache || lower_part == 0) {
            ...
            req.q = "";
            req.peer = MessagesController.getInstance(Const.currentAccount).getInputPeer(lower_part);
            if (req.peer == null) {
                return;
            }
            int reqId = ConnectionsManager.getInstance(Const.currentAccount).sendRequest(req, new RequestDelegate() {
                @Override
            ...
            ConnectionsManager.getInstance(Const.currentAccount).bindRequestToGuid(reqId, classGuid);
        }
    }
        ...
        if (fromCache || lower_part == 0) {
            ...
            int reqId = ConnectionsManager.getInstance(Const.currentAccount).sendRequest(req, new RequestDelegate() {
                ...
                public void run(TLObject response, TLRPC.TL_error error) {
                    if (error == null) {
                        final TLRPC.messages_Messages res = (TLRPC.messages_Messages) response;
                        MessagesStorage.getInstance(Const.currentAccount).putUsersAndChats(res.users, res.chats, true, true);
                        int count;
                        ...
                        AndroidUtilities.runOnUIThread(new Runnable() {
                            @Override
                            public void run() {
                                MessagesController.getInstance(Const.currentAccount).putUsers(res.users, false);
                                MessagesController.getInstance(Const.currentAccount).putChats(res.chats, false);
                            }
                        });
    ...
    private static void processLoadedMedia(final TLRPC.messages_Messages res, final long uid, int offset, int count, int max_id, final int type, final boolean fromCache, final int classGuid, final boolean isChannel, final boolean topReached) {
        ...
        if (fromCache && res.messages.isEmpty() && lower_part != 0) {
            ...
            for (int a = 0; a < res.messages.size(); a++) {
                ...
                objects.add(new MessageObject(Const.currentAccount,message, usersDict, true));
            }
            ...
            AndroidUtilities.runOnUIThread(new Runnable() {
                ...
                public void run() {
                    ...
                    MessagesController.getInstance(Const.currentAccount).putUsers(res.users, fromCache);
                    MessagesController.getInstance(Const.currentAccount).putChats(res.chats, fromCache);
                    NotificationCenter.getInstance(Const.currentAccount).postNotificationName(NotificationCenter.mediaDidLoad, uid, totalCount, objects, classGuid, type, topReached);
                }
            });
        }
    }
        ...
        AndroidUtilities.runOnUIThread(new Runnable() {
            ...
            public void run() {
                ...
                if (fromCache && count == -1 && lower_part != 0) {
                    ...
                    NotificationCenter.getInstance(Const.currentAccount).postNotificationName(NotificationCenter.mediaCountDidLoad, uid, (fromCache && count == -1 ? 0 : count), fromCache, type);
                }
            }
        });
    }
    ...
    private static void putMediaCountDatabase(final long uid, final int type, final int count) {
        MessagesStorage.getInstance(Const.currentAccount).getStorageQueue().postRunnable(new Runnable() {
            @Override
            public void run() {
                try {
                    SQLitePreparedStatement state2 = MessagesStorage.getInstance(Const.currentAccount).getDatabase().executeFast("REPLACE INTO media_counts_v2 VALUES(?, ?, ?)");
                    state2.requery();
    ...
    private static void getMediaCountDatabase(final long uid, final int type, final int classGuid) {
        MessagesStorage.getInstance(Const.currentAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                try {
                    int count = -1;
                    SQLiteCursor cursor = MessagesStorage.getInstance(Const.currentAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT count FROM media_counts_v2 type = %d LIMIT 1", type));
                    if (cursor.next()) {
                    ...
                    if (count == -1 && lower_part == 0) {
                        cursor = MessagesStorage.getInstance(Const.currentAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT COUNT(mid) FROM media_v2 type = %d LIMIT 1", type));
                        if (cursor.next()) {
                            count = cursor.intValue(0);
                        }
                        cursor.dispose();
        ...
        MessagesStorage.getInstance(Const.currentAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    SQLiteCursor cursor;
                    SQLiteDatabase database = MessagesStorage.getInstance(Const.currentAccount).getDatabase();
                    boolean isEnd = false;
                    if ((int) uid != 0) {
                        int channelId = 0;
                    ...
                    if (!usersToLoad.isEmpty()) {
                        MessagesStorage.getInstance(Const.currentAccount).getUsersInternal(TextUtils.join(",", usersToLoad), res.users);
                    }
                    if (!chatsToLoad.isEmpty()) {
                        MessagesStorage.getInstance(Const.currentAccount).getChatsInternal(TextUtils.join(",", chatsToLoad), res.chats);
                    }
                    if (res.messages.size() > count) {
                        topReached = false;
                        res.messages.remove(res.messages.size() - 1);
            ...
            public void run() {
                try {
                    if (messages.isEmpty() || topReached) {
                        MessagesStorage.getInstance(Const.currentAccount).doneHolesInMedia(uid, max_id, type);
                        if (messages.isEmpty()) {
                            return;
                        }
                    }
                    MessagesStorage.getInstance(Const.currentAccount).getDatabase().beginTransaction();
                    SQLitePreparedStatement state2 = MessagesStorage.getInstance(Const.currentAccount).getDatabase().executeFast("REPLACE INTO media_v2 VALUES(?, ?, ?, ?, ?)");
                    ...
                    if (!topReached || max_id != 0) {
                        ...
                        if (max_id != 0) {
                            MessagesStorage.getInstance(Const.currentAccount).closeHolesInMedia(uid, minId, max_id, type);
                        } else {
                            MessagesStorage.getInstance(Const.currentAccount).closeHolesInMedia(uid, minId, Integer.MAX_VALUE, type);
                        }
                    }
    ...
    public static void loadMusic(final long uid, final int max_id) {
        MessagesStorage.getInstance(Const.currentAccount).getStorageQueue().postRunnable(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    SQLiteCursor cursor = MessagesStorage.getInstance(Const.currentAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT data, mid FROM media_v2 mid < %d AND type = %d ORDER BY date DESC, mid DESC LIMIT 1000", max_id, MEDIA_MUSIC));
                    ...
                    while (cursor.next()) {
                        ...
                        if (data != null) {
                            ...
                            if (MessageObject.isMusicMessage(message)) {
                                message.id = cursor.intValue(1);
                                message.dialog_id = uid;
                                arrayList.add(0, new MessageObject(Const.currentAccount,message, false));
                            }
                        }
                        data.reuse();
                    }
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    @Override
                    public void run() {
                        NotificationCenter.getInstance(Const.currentAccount).postNotificationName(NotificationCenter.musicDidLoad, uid, arrayList);
                    }
                });
            }
        });
