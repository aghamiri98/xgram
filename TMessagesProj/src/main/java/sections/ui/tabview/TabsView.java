...
public class TabsView extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    private void unreadCount() {
        if (!Theme.plusHideUnreadTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsUnread, positions[8]);
        if (!Theme.plusHideAdminTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsadmins, positions[7]);
        if (!Theme.plusHideFavsTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsFavs, positions[6]);
        if (!Theme.plusHideBotsTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsBots, positions[5]);
        if (!Theme.plusHideChannelsTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsChannels, positions[4]);
        unreadCountGroups();
        if (!Theme.plusHideUsersTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsUsers, positions[1]);
        if (!Theme.plusHideAllTab)
            unreadCountAll(MessagesController.getInstance(currentAccount).dialogs, positions[0]);
    }

    private void unreadCountGroups() {
        if (!Theme.plusHideGroupsTab)
            unreadCount(!Theme.plusHideSuperGroupsTab ? MessagesController.getInstance(currentAccount).dialogsGroups : MessagesController.getInstance(currentAccount).dialogsGroupsAll, positions[2]);
        if (!Theme.plusHideSuperGroupsTab)
            unreadCount(MessagesController.getInstance(currentAccount).dialogsMegaGroups, positions[3]);
    }
